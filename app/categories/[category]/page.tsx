...
export default function CategoryPage({ params }: { params: { category: string } }) {
  ...
  const category = categoryData.find((cat) => cat.id === categorySlug) || {
    id: categorySlug,
    name: categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1),
    description: "Articles related to this category",
  ...
  return (
    <>
      <CategorySchema category={category} postCount={filteredPosts.length} />
      <Navigation />
        ...
        <div className="container mx-auto px-4 py-24 md:py-32">
          ...
          <div className="text-center mb-16">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p className="text-lg text-neutral-700 dark:text-neutral-300 max-w-2xl mx-auto">{category.description}</p>
            </motion.div>
          </div>
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end">
              <div className="container mx-auto px-4 py-8">
                <h2 className="text-white text-2xl md:text-3xl font-light">
                  {filteredPosts.length} {filteredPosts.length === 1 ? "Article" : "Articles"} in {category.name}
                </h2>
              </div>
            </div>
          </div>
