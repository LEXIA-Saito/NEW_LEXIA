...
// Append Jujutsu (jj) explainer — Git-compatible VCS, working-copy-as-a-commit, op log, adoption criteria
fallbackBlogPosts.push({
  slug: "what-is-jujutsu-jj-vcs",
  title: "Jujutsu（jj）とは？Git互換の新しいバージョン管理システム｜作業コピー＝コミットという発想",
  description:
    "Gitリポジトリをそのまま使いながら、ステージングもstashも不要にするバージョン管理システム「Jujutsu（jj）」を解説します。作業コピーがコミットになる仕組み、操作ログによるundo、既存Gitリポジトリへの導入手順と制約を一次情報で整理します。",
  genre: "Full-stack",
  ...
  sections: [
    ...
    {
      heading: "Jujutsuとは？Gitリポジトリをそのまま使う別のバージョン管理システム",
      body: [
    ...
    {
      heading: "Gitとの違い①：作業コピーがコミットで、ステージングエリアがない",
      body: [
      ...
      table: {
        headers: ["やりたいこと", "Gitでの操作", "Jujutsuでの操作"],
        rows: [
    ...
    {
      heading: "Gitとの違い②：操作ログによるundoと、コンフリクトを抱えたままのコミット",
      body: [
    ...
    {
      heading: "導入方法：既存のGitリポジトリにそのまま重ねる",
      body: [
    ...
    {
      heading: "まとめ：手元だけで試せるので、判断は実際に触ってからでよい",
      body: [
...
fallbackBlogPosts.push({
  slug: "what-is-datastar-hypermedia-framework",
  title:
    "Datastarとは？data-*属性とSSEでUIを動かす約12KBのハイパーメディアフレームワーク｜状態はバックエンドに置く",
  description:
    "htmxのようなバックエンド駆動と、Alpine.jsのようなフロントの小さな反応を1本のスクリプトにまとめたDatastarを解説します。シグナルとdata-*属性の書き方、SSEでDOMを差し替える仕組み、CSPやSSE運用の制約、有償Pro版の費用まで一次情報で整理します。",
  genre: "Frontend",
  tags: ["Datastar", "Hypermedia", "OSS"],
  date: "2026-08-30",
  sections: [
    {
      body: [
        "社内向けの管理画面に、一覧の絞り込みと保存中のスピナー、それに他の担当者が更新したら自分の画面にも反映される仕組みを足したい。ReactやNext.jsを一式持ち込むほどの規模ではないけれど、素のJavaScriptで書くと状態の置き場所がすぐ散らかる。この中間を埋めようとすると、部分更新はhtmx、細かい表示の出し分けはAlpine.js、という二枚看板に落ち着きがちです。",
    ...
    {
      heading: "Datastarとは？画面の状態をバックエンドに寄せるハイパーメディアフレームワーク",
      body: [
    ...
    {
      heading: "仕組み①：シグナルとdata-*属性で、書くJavaScriptを減らす",
      body: [
      ...
      table: {
        headers: ["やりたいこと", "書く属性", "起きること"],
        rows: [
          ["入力欄と状態を結ぶ", "data-bind:foo", "$foo というシグナルが作られ、入力と双方向に同期する"],
          ["状態を画面に出す", "data-text", "式の評価結果を要素のテキストにする"],
          ["条件で出し分ける", "data-show / data-class:success", "式の真偽で表示を切り替える、クラスを付け外しする"],
          ["派生した値を持つ", "data-computed:repeated", "他のシグナルから計算される読み取り専用のシグナルを作る"],
          ["イベントを拾う", "data-on:click", "クリックなどのイベントでDatastar式を実行する"],
          ["通信中を示す", "data-indicator", "リクエストが飛んでいる間だけ true になるシグナルを作る"],
        ],
      },
    },
    {
      heading: "仕組み②：バックエンドがHTMLとシグナルを送り込んでDOMを書き換える",
      body: [
    ...
    {
      heading: "導入方法：scriptタグ1本と、使っている言語のSDK",
      body: [
    ...
    },
    {
      ...
      table: {
        headers: ["ライセンス", "価格（2026年8月時点）", "対象"],
        rows: [
          ["Solo", "349ドル（通常399ドル）", "個人開発者・フリーランス、開発者が1人だけの小規模組織"],
          ["Team", "1,299ドル（通常1,499ドル）", "従業員25人以下の組織"],
          ["Enterprise", "個別見積もり", "従業員25人を超える組織。専任サポートや個別のライセンス条件が付く"],
        ],
      },
    },
    {
      heading: "まとめ：SSRの延長で動くものを作りたいときの選択肢",
      body: [
    ...
    },
    {
      ...
      list: [
        ...
        "Datastar Pro（価格とライセンス）",
        "https://data-star.dev/pro",
        "GitHub リポジトリ（README・ライセンス）",
        "https://github.com/starfederation/datastar",
        "リリース一覧（v1.0.0・v1.0.3 のリリースノート）",
