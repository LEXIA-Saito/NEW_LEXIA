...
// Append Agent Client Protocol (ACP) explainer — editor/agent interop standard, JSON-RPC lifecycle, MCP relationship, registry, v1/v2 status
fallbackBlogPosts.push({
  slug: "what-is-agent-client-protocol-acp",
  title: "Agent Client Protocol（ACP）とは？AIエージェントとエディタをつなぐ共通規格を仕組みから読む",
  description:
    "エディタとAIコーディングエージェントの組み合わせごとに個別実装が必要だった問題を標準化で解こうとするAgent Client Protocol（ACP）を解説します。JSON-RPCベースの通信モデル、MCPとの役割分担、ACPレジストリからの導入手順、v2ドラフトとガバナンスの現状までを一次情報で整理します。",
  genre: "AI",
  tags: ["ACP", "AIエージェント", "OSS"],
  date: "2026-08-29",
  sections: [
    {
      body: [
    ...
    {
      heading: "ACPとは？エディタとエージェントの通信を標準化するオープンな規格",
      body: [
    ...
    {
      heading: "仕組み：JSON-RPCで会話し、環境の鍵はエディタが握る",
      body: [
    ...
    {
      heading: "MCPとの違い：競合ではなく、担当する区間が違う",
      body: [
    ...
    {
      heading: "導入：ACPレジストリからエージェントを入れる",
      body: [
    ...
    {
      heading: "まとめ：エディタとエージェントの乗り換えコストを切り離す試み",
      body: [
